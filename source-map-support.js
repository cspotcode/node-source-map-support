...
var sharedData = initializeSharedData({
  ...
  /** @type {Record<string, {url: string, map: SourceMapImpl}} */
...
function mapSourcePosition(position) {
  return _mapSourcePosition(new Position(position.source, position.line, position.column));
}
/** @typedef {{
 *   source: string;
 *   line: number;
...
function _mapSourcePosition(position) {
  ...
  if (!sourceMap) {
    ...
    if (urlAndMap) {
      sourceMap = setSourceMapCache(position.source, {
        url: urlAndMap.url,
        map: createMap(urlAndMap.map, urlAndMap.url)
      });
      ...
      // if (sourceMap.map.sourcesContent) {
        for(let i = 0; ; i++) {
          const source = sourceMap.map.getSource(i);
          if(!source) break;
          var contents = getSourceMapSourceContent(sourceMap.map, source, i);
          if (contents) {
  ...
  if (sourceMap && sourceMap.map) {
    var originalPosition = getOriginalPositionFor(sourceMap.map, position);
    ...
    if (originalPosition?.source != null) {
      position.source = supportRelativeURL(
        sourceMap.url, originalPosition.source);
      position.line = originalPosition.original.line;
      position.column = originalPosition.original.column;
      position.name = originalPosition.name;
      return position;
    }
  }
...
function mapEvalOrigin(origin) {
  ...
  if (match) {
    var position = _mapSourcePosition(new Position(
      match[2],
      +match[3],
      match[4] - 1
    ));
...
//#region sourcemap consumer
/** @typedef {SourceMap} SourceMapImpl */
function createMap(map, url) {
  // return new AnyMap(map, url);
  const sm = new SourceMap();
  sm.addVLQMap(JSON.parse(map));
  return sm;
}
/**
 * @param {SourceMapImpl} map 
 * @param {Position} position 
 */
function getOriginalPositionFor(map, position) {
  // return originalPositionFor(map, position);
  return map.findClosestMapping(position.line, position.column);
}
/**
 * @param {SourceMapImpl} map
 * @param {string} source
 * @param {number} index
 */
function getSourceMapSourceContent(map, source, index) {
  return map.getSourceContent(source);
  // return sourceMap.map.sourcesContent[i];
}
